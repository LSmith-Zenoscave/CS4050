...
autoComplete :: STrie -> String -> [String]
autoComplete t s = maybe [] allPrefixes $ findPrefix s t

makeTrie :: String -> STrie
makeTrie s = ifold insert (lines s) emptyTrie


prompt :: STrie -> String -> IO String
prompt t p = do c <- getChar
                if c == '\n'
                  then prompt t ""
                  else do putStrLn (p ++ [c] ++ ": " ++ intercalate "\n" (take 10 (autoComplete t (p ++ [c]))))
                          prompt t (p ++ [c])


main :: IO ()
main =  do f <- getArgs >>= readFile . head
           prompt (makeTrie f) ""
           return ()
