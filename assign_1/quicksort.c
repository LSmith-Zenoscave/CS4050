...
Pivot pivots[3] = {{first, "First Value"}, {median, "Median of 3"}, {rand_pivot, "Random value"}};
Partition partitions[2] = {{hoare, "Hoare"}, {lomuto, "Lomuto"}};
Callback callbacks[2] = {{Id, 2, "No Callback"}, {InsertionSort, 10, "Insertion Sort"}};
...
int main(int argc, char *argv[])
{
    ...
    int trial = 1;
    int trials = windows * repeats * pivot_count * partition_count * callback_count;
    ...
    for (int size = range; size <= windows * range; size += range)
    {
        ...
        for (int pivot = 0; pivot < pivot_count; pivot++)
        {
            for (int part = 0; part < partition_count; part++)
            {
                ...
                {
                    QuickSorter config = {pivots[pivot], partitions[part], callbacks[call]};
                    double time = 0;
                    for (int n = 1; n <= repeats; n++)
                    {
                        fprintf(stderr, "\rTrial # %d / %d done. (%0.2f%% of all values) ", trial, trials, 100.0 * pow((double)trial / trials, 2.0));
                        ...
                        for (int idx = 0; idx < size - 1; idx++)
                        {
                            ...
                            {
                                fprintf(stderr, "error after sorting: tmp[%d] > tmp[%d], (%d > %d)\n.", idx, idx + 1, tmp[idx], tmp[idx + 1]);
                                exit(EXIT_FAILURE);
                            }
                        }
                    ...
                    printf("%s,%s,%s,%d,%f\n", pivots[pivot].name, partitions[part].name, callbacks[call].name, size, time);
                }
            }
        }
        free(arr);
